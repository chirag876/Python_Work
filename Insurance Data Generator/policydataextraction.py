...
total_premium = np.round(np.random.uniform(1000, 10000, num_records), 2)
claim_Amount=np.round(total_premium-500,2)

# Generate policy durations
original_Effective_Date=['2022-11-04'] * num_records
policy_effective_dates = pd.date_range(start='2020-01-01', periods=num_records, freq='M')
...
data.to_csv('C:\WorkSpaces\CodeSpaces\DataGenerator\Insurance Data Generator\csv\PolicyDataExtraction\policy_data_extraction_test.csv', index=False)
